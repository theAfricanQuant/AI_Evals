...
6. Deliverables presented to the user in the topic render as self-contained HTML in the owner's website theme (sisengai.com "LivingStory": cream `#FDFBF7`, mahogany `#321208`, amber `#D4720A`, Inter + Playfair Display), using the shared template in `skills/ai-evals-session/`. Store and track every report at `reports/generated/chNN-exNN-topic-YYYY-MM-DD.html`; the chapter comes first so related reports stay together, and the date comes last so later clarifications can be added without breaking the chapter grouping. Reports are part of the repository baseline and may be committed and pushed.
